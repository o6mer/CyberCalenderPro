...
function SingleClass() {
    ...
    useEffect(()=>{
        setIsLoading(true)
        axios.post("http://localhost:2000/getdatedata", {className: name, date:todayFormat}).then((classDate)=>{
            console.log("today data: ",classDate.data)
    ...
    todayEvents.map((oneEvent,index)=>{
        rows.push({date:oneEvent.date,time_range:oneEvent.time_range,approved:oneEvent.approved
            ,id:index,By:oneEvent.users[0].userName,phone:oneEvent.users[0].phoneNumber})
    })
    return (
    <>
    <NavBar name={name}/>
        <div style={{ height: 400, width: '100%' }}>
            <DataGrid sx={{overflow:"hidden",textAlign:"center"}}
                rows={rows}
                columns={columns}
                pageSize={5}
